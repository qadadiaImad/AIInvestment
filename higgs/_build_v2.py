...
def soft_hook(kick,head,sub,foot="not financial advice  ·  swipe &rarr;"):
    ...
    <div style="font-family:Fraunces;font-weight:600;font-size:92px;line-height:1.03;letter-spacing:-2px;margin-top:30px">{head}</div>
    <div style="font-size:33px;line-height:1.42;color:#5C6270;margin-top:auto;max-width:800px">{sub}</div>
...
def soft_takeaway(kick,big,sub):
    ...
    <div style="font-family:Fraunces;font-weight:700;font-size:84px;line-height:1.04;letter-spacing:-2px;margin-top:18px;color:#15171C">{big}</div>
...
def dark_number(kick,big,unit,lab,sub):
    ...
    <div style='flex:1;display:flex;flex-direction:column;justify-content:center'>
      ...
      <div style="font-family:Fraunces;font-weight:700;font-size:220px;line-height:.9;letter-spacing:-6px;margin-top:30px;background:linear-gradient(180deg,#FFFFFF,#9FE7C8);-webkit-background-clip:text;-webkit-text-fill-color:transparent">{big}<span style='font-size:116px'>{unit}</span></div>
      <div style="font-family:'JetBrains Mono';font-size:28px;color:#8A95A5;letter-spacing:2px;margin-top:14px">{lab}</div>
      <div style="font-size:34px;line-height:1.42;color:#C2CAD6;margin-top:46px;max-width:840px">{sub}</div>
    </div>
...
def soft_disclosure(rows,kick,title,note):
    rh="".join(f"<div style='display:flex;margin:18px 0'><div style=\"width:230px;font-family:'JetBrains Mono';font-size:23px;color:#8A8170\">{k}</div><div style='font-family:Inter;font-weight:600;font-size:27px;color:#1A1C22'>{v}</div></div>" for k,v in rows)
    b=f"""<div class='pad'><div class='kick' style='color:#B4690E'><span class='dot' style='background:#B4690E'></span>{kick}</div>
    <div style="font-family:Fraunces;font-weight:600;font-size:62px;line-height:1.04;letter-spacing:-1px;margin-top:26px">{title}</div>
    <div style='background:#FFFCF6;border:1.5px solid #E4DCCB;border-radius:18px;padding:40px 44px;margin-top:40px'>
    <div style="font-family:'JetBrains Mono';font-weight:700;font-size:22px;letter-spacing:2px;color:#B4690E;margin-bottom:14px">STOCK ACT DISCLOSURE</div>{rh}</div>
    <div style='font-size:28px;color:#5C6270;margin-top:auto'>{note}</div>
    <div class='foot' style='margin-top:26px'>public disclosure for transparency — not an accusation, not a signal · NFA</div></div>"""
    return page(b,SOFT_CSS,"soft")

# ---- compute fresh numbers ----
crm=site['stocks']['CRM']['valuation']; CRM_DC=crm['fundamental_discount_pct']
ionq=qs['IONQ']['valuation']; IONQ_DC=ionq['fundamental_discount_pct']
sal=next((t for t in cg['trades'] if t.get('ticker')=='CRM' and 'salazar' in (t.get('politician') or '').lower()),None)

SL={
 "post1_1_hook.png":soft_hook("AI &nbsp;·&nbsp; VALUATION","$50M <span style='font-family:JetBrains Mono;font-weight:700'>&rarr;</span> <em style='font-style:italic;color:#0E9F6E'>$5B.</em><br>The stock didn&rsquo;t<br>get the memo.","Salesforce&rsquo;s small AI bet is reportedly worth billions now. The win gets reshared; almost no one checks the price."),
 "post1_2_graph.png":dark_chart("CRM","CRM &nbsp;·&nbsp; PRICE vs FUNDAMENTAL VALUE","Salesforce &mdash; price vs<br>fundamental value",f"trading <span style='color:#34D399;font-weight:700'>~{abs(CRM_DC):.0f}% below</span> analysts&rsquo; model"),
 "post1_3_takeaway.png":dark_number("THE GAP",f"~{abs(CRM_DC):.0f}","%","BELOW ANALYSTS' FUNDAMENTAL VALUE","The win and the price are telling two different stories. Not that the model is right &mdash; just that they disagree."),
 "post2_1_hook.png":soft_hook("QUANTUM","Quantum stocks<br>aren&rsquo;t <em style='font-style:italic;color:#0E9F6E'>one trade.</em>","Same hype label. Wildly different math &mdash; one name even trades below what analysts model."),
 "post2_2_graph.png":dark_multiples(["IONQ","QUBT","QBTS","RGTI"],"QUANTUM &nbsp;·&nbsp; PRICE vs MODEL","Price vs fundamental value","times the price sits above analysts&rsquo; fundamental value","IonQ is the only one trading below the model."),
 "post2_3_takeaway.png":dark_number("THE OUTLIER",f"~{abs(IONQ_DC):.0f}","%","IonQ BELOW MODEL — PEERS 10–30x ABOVE","Record revenue was reported this quarter (reported, not confirmed). The rest is narrative-priced. Not a recommendation."),
 "post3_1_hook.png":soft_hook("PUBLIC RECORD","A House member<br>disclosed buying the<br>stock <em style='font-style:italic;color:#B4690E'>analysts call cheap.</em>","Straight from the STOCK Act filing. No spin.","not financial advice  ·  swipe &rarr;"),
 "post3_2_graph.png":soft_disclosure([("Member","Rep. Maria Elvira Salazar"),("Security","Salesforce (CRM)"),("Type","Purchase"),("Amount","$15,001 – $50,000"),("Traded","Apr 6, 2026"),("Filed","May 8, 2026")],"PUBLIC RECORD","What the filing<br>actually says",f"The same CRM analysts flag ~{abs(CRM_DC):.0f}% below fundamental value."),
 "post3_3_takeaway.png":soft_takeaway("TRANSPARENCY","Public record —<br>not a signal.","Not an accusation, not insider anything, not advice. Just who disclosed what, and when."),
}
with sync_playwright() as p:
    ...
    b.close()
print("DONE  CRM_DC",CRM_DC,"IONQ_DC",IONQ_DC,"salazar",bool(sal))
