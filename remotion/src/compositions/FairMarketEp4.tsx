...
import {BigNumberExhibit, CollapseExhibit, ColumnsExhibit, TickerTape} from "../motion/Infographic";
import anchors from "../fixtures/cast_ep1/pose_anchors.json";
import mouthTracks from "../fixtures/cast_ep1/mouth_tracks_ep4.json";
...
export const EP4_FRAMES = 4200;   // 140s
...
type Beat = {
  ...
  graphic?: "lic_10b" | "gap_bars" | "collapse_95" | "three_nouns";
...
const BEATS: Beat[] = [
  ...
  {at: 2890, actors: [{poses: ["sol_smug_v1"], kind: "bust", x: 560, y: 1180, h: 880},
   ...
   shots: [{from: 0, only: 0, k: 1.02, kEnd: 1.1, tx: 540, ty: 1200},
           {from: 117, only: 1, k: 1.02, kEnd: 1.1, tx: 322, ty: 1240}],
  ...
  {at: 3240, actors: [{poses: ["sol_finger"], kind: "full", x: 800, y: FLOOR_Y, h: 900},
   ...
   shots: [{from: 0, only: 0, k: 1.02, kEnd: 1.12, tx: 580, ty: 1190},
           {from: 99, only: 1, k: 1.02, kEnd: 1.1, tx: 322, ty: 1240}],
  ...
  {at: 3400, actors: [{poses: ["sol_point"], kind: "full", x: 790, y: FLOOR_Y, h: 930},
   ...
   shots: [{from: 0, only: 0, k: 1.0, kEnd: 1.06},
           {from: 148, only: 1, k: 1.02, kEnd: 1.1, tx: 322, ty: 1240}],
...
const beatAt = (f: number) => {
  ...
  return {cur, since: f - cur.at, hold: (next ? next.at : EP4_FRAMES) - cur.at};
};
...
export const FairMarketEp4: React.FC = () => {
        ...
        <React.Fragment key={b.at}>
          {b.vo ? (
            <Sequence from={b.at + VO_DELAY} durationInFrames={320}>
              <Audio src={staticFile(`audio/fairmarket_ep4/${b.vo}.wav`)} />
            </Sequence>
          ) : null}
          {b.vo2 ? (
            <Sequence from={b.at + (b.at2 ?? 0) + VO_DELAY} durationInFrames={120}>
              <Audio src={staticFile(`audio/fairmarket_ep4/${b.vo2}.wav`)} />
            </Sequence>
          ) : null}
          {(b.sfx ?? []).map((s, i) => (
          ...
          {shot?.hideCard ? <TVIdle frame={frame} bare={!!cur.title} />
            : shot?.tvPhoto ? (
              ...
              <BigNumberExhibit since={since} w={WALL.w} h={WALL.h}
                kicker="EXPORT LICENCES — APPROVED"
                value="$10B+"
                caption="approved is a licence category — not a sale"
                foot="as reported · U.S. export licences for China-bound chips" />
            ) : cur.graphic === "gap_bars" ? (
              <ColumnsExhibit since={since} w={WALL.w} h={WALL.h}
                title="APPROVED vs SHIPPED"
                cols={[{label: "APPROVED", sub: "$10B+", frac: 1.0},
                       {label: "SHIPPED", sub: '"very few"', frac: 0.06,
                        tone: "red"}]}
                foot={'"very few" — the Commerce Department’s words to Congress · as reported'} />
            ) : cur.graphic === "collapse_95" ? (
              <CollapseExhibit since={since} w={WALL.w} h={WALL.h}
                title="THE CHINA DATA-CENTER MARKET"
                fromPct={95}
                caption={'from ~95% share and $17B of revenue to "effectively foreclosed" — the company’s own filing’s words'}
                foot="share, revenue and filing language as reported · categories as named" />
            ) : cur.graphic === "three_nouns" ? (
              <ColumnsExhibit since={since} w={WALL.w} h={WALL.h}
                title="THREE NOUNS, THREE NUMBERS"
                cols={[{label: "APPROVED", sub: "$10B+", frac: 1.0},
                       {label: "SHIPPED", sub: '"very few"', frac: 0.06,
                        tone: "red"},
                       {label: "BOOKED", sub: "not yet revenue", frac: 0.03,
                        tone: "red"}]}
                foot={"a government word, a business word, an accountant’s word · never the same number"} />
