...
import mouthTracks from "../fixtures/cast_ep1/mouth_tracks_bshort.json";
...
const BEATS: Beat[] = [
  ...
  {at: 1118,
   ...
   actors: [{poses: ["sol_finger"], kind: "full", x: 790, y: FLOOR_Y, h: 930}],
   shots: [{from: 0, k: 1.0, kEnd: 1.1}],
   vo: "s13_sol_watch", speaker: "SOL",
   line: "Gone. Watch the machine, not the number."},
];
...
const beatAt = (f: number) => {
  ...
  return {cur, idx: i, since: f - cur.at,
    hold: (next ? next.at : BUBBLES_SHORT_FRAMES) - cur.at};
};
...
export const BubblesShort: React.FC = () => {
        ...
        <React.Fragment key={b.at}>
          {b.vo ? (
            <Sequence from={b.at + VO_DELAY} durationInFrames={320}>
              <Audio src={staticFile(`audio/fairmarket_bshort/${b.vo}.wav`)} />
            </Sequence>
          ) : null}
          {b.vo2 ? (
            <Sequence from={b.at + (b.at2 ?? 0) + VO_DELAY} durationInFrames={120}>
              <Audio src={staticFile(`audio/fairmarket_bshort/${b.vo2}.wav`)} />
            </Sequence>
          ) : null}
