...
// ---------------------------------------------------------------- FRAMING
// Shared with ep.2 and kept identical on purpose - a series that reframes
// itself between episodes reads as an accident. The staging was authored
// phone-first, where sitting close to the desk feels intimate; on a YouTube
// page it reads as crowded, with the cast filling the frame and the studio
// gone.
//
//   CAM_PULL  scales how far each authored push-in travels. The camera goes
//             where the beat says, just not as far. 0.45 turns a 1.30x push
//             into 1.135. It cannot usefully go below 1.0 at rest: the room
//             is drawn to fill the frame exactly, so zooming past that
//             reveals the canvas edge.
//   CAST      scales every staged actor height. Feet stay on FLOOR_Y, so
//             figures shrink into the set rather than floating, and the
//             screenboard above them gains room.
const CAM_PULL = 0.45;
const CAST = 0.88;
const VO_DELAY = 6;
...
type Beat = {
  ...
  // a generated caricature plate on the wall (motion/ExhibitPlate)
  exhibit?: string;
  // Force the live candlestick tape back onto the wall. A beat with no
  // wall content of its own now HOLDS the exhibit it is reacting to
  // instead of cutting back to the tape - 23 of 39 beats used to fall
  // through to it, so the same chart was on screen for 59% of the
  // episode. Exactly one beat carries `wall: "chart"`.
  wall?: "chart";
  callout?: {x: number; y: number; at?: number; label?: string};
  shots?: Shot[];
...
const BEATS: Beat[] = [
  // ═══ ACT 1 — THE CLAIM ═══════════════════════════════════════════════
  {at: 0,
   exhibit: "capitol_ticker", title: ["MARKET LESSONS", "WITH SOL", "ep.1 — the 'fair' market"],
  ...
  {at: 122,
   exhibit: "fundamentals_ledger",
   actors: [{poses: ["rex_eager"], kind: "full", x: 327, y: FLOOR_Y, h: 717,
  ...
  {at: 217, actors: [{poses: ["sol_laugh"], kind: "full", x: 814, y: FLOOR_Y, h: 622},
   ...
   sfx: [{at: 15, name: "faah", vol: 0.28}, {at: 18, name: "sfx_whip", vol: 0.38}],
  ...
  {at: 299,
   exhibit: "deal_handshake",
   callout: {x: 0.46, y: 0.55, at: 30, label: "THE DEAL"}, actors: [{poses: ["rex_listen"], kind: "full", x: 236, y: FLOOR_Y, h: 659},
   ...
   sfx: [{at: 20, name: "core", vol: 0.3}, {at: 4, name: "sfx_whoosh", vol: 0.45}],
  ...
  {at: 393,
   sfx: [{at: 0, name: "vine_boom_bass", vol: 0.3}], actors: [{poses: ["rex_shock"], kind: "full", x: 520, y: FLOOR_Y, h: 900}],
  ...
  {at: 859,
   ...
   exhibit: "leaderboard_suits", actors: [{poses: ["sol_smug_v1"], kind: "bust", x: 560, y: 1120, h: 1040}],
  ...
  {at: 1301,
   exhibit: "committee_room", actors: [{poses: ["sol_finger"], kind: "full", x: 760, y: FLOOR_Y, h: 900}],
  ...
  {at: 1796,
   exhibit: "obvious_idea",
   actors: [{poses: ["sol_smug_v1"], kind: "bust", x: 560, y: 1120, h: 1040}],
  ...
  {at: 1892,
   sfx: [{at: 6, name: "among_us", vol: 0.3}],
   exhibit: "index_basket", actors: [{poses: ["rex_eager"], kind: "full", x: 560, y: FLOOR_Y, h: 930}],
  ...
  {at: 1970,
   exhibit: "two_podiums", actors: [{poses: ["sol_point"], kind: "full", x: 790, y: FLOOR_Y, h: 930}],
  ...
  {at: 2240,
   exhibit: "calendar_late",
   callout: {x: 0.52, y: 0.6, at: 34, label: "45 DAYS"}, actors: [{poses: ["sol_point_v1"], kind: "full", x: 745, y: FLOOR_Y, h: 930},
  ...
  {at: 2417,
   sfx: [{at: 15, name: "vine_boom", vol: 0.24}],
   exhibit: "copy_homework", actors: [{poses: ["rex_eager"], kind: "full", x: 312, y: FLOOR_Y, h: 679},
  ...
  {at: 2537,
   sfx: [{at: 0, name: "whoosh_fire", vol: 0.3}], actors: [{poses: ["sol_finger"], kind: "full", x: 834, y: FLOOR_Y, h: 597},
  ...
  {at: 2722,
   sfx: [{at: 20, name: "core", vol: 0.33}], actors: [{poses: ["sol_point"], kind: "full", x: 790, y: FLOOR_Y, h: 930},
  ...
  // WHY the copy underperforms, and it is not the timing alone. The
  // disclosed positions were concentrated megacap technology and a
  // material part of the upside came through LONG-DATED CALL OPTIONS —
  // leverage. A filing gives you the name. It does not give you the
  // instrument, the size or the entry, and a diversified fund holding
  // shares cannot reproduce any of the three.
  //
  // This is also where the episode declines to say the thing it would be
  // easiest to say. "The gains come from inside information" is an
  // accusation of a crime against identifiable people, it is not what
  // these filings show, and the fund's 2026 drawdown argues against a
  // durable information edge anyway — an edge does not reverse when its
  ...
  {at: 3664,
   exhibit: "options_leverage", actors: [{poses: ["sol_point"], kind: "full", x: 790, y: FLOOR_Y, h: 930},
                      {poses: ["rex_skeptic"], kind: "bust", x: 430, y: 1250, h: 860}],
   card: {title: "WHAT A FILING DOESN'T SHOW",
          lines: ["The name — yes.",
                  "The instrument — calls, not shares.",
                  "The size, and the entry — no."],
          foot: "disclosed positions included long-dated call options"},
   shots: [{from: 0, only: 0, k: 1.0, kEnd: 1.06},
           {from: 150, only: 0, k: 1.2, kEnd: 1.32, tx: 580, ty: 1190, hideCard: true},
           {from: 224, only: 1, k: 1.0, kEnd: 1.1, hideCard: true}],
  ...
  {at: 4007,
   ...
   shots: [{from: 0, k: 1.0, kEnd: 1.08},
           {from: 62, only: 0, k: 1.3, kEnd: 1.42, tx: 520, ty: 1030}],
   vo: "a6_rex_getit", speaker: "REX",
  ...
  {at: 4326,
   exhibit: "redacted_filing",
   actors: [{poses: ["sol_smug_v1"], kind: "bust", x: 560, y: 1180, h: 900}],
  ...
  {at: 4626,
   exhibit: "attention_map",
   actors: [{poses: ["sol_finger"], kind: "full", x: 840, y: FLOOR_Y, h: 578,
  ...
  {at: 4906,
   exhibit: "watching_chart", actors: [{poses: ["rex_skeptic"], kind: "full", x: 312, y: FLOOR_Y, h: 679,
   ...
   sfx: [{at: 14, name: "sfx_pop", vol: 0.6}, {at: 24, name: "sfx_whip", vol: 0.42}],
  ...
  {at: 5296,
   exhibit: "oil_tanker_strait", actors: [{poses: ["rex_eager"], kind: "full", x: 312, y: FLOOR_Y, h: 679,
                       turns: [{at: 12, tx: 830, ty: 1230}]},
                      {poses: ["sol_smug_v1"], kind: "bust", x: 812, y: 1300, h: 596}],
   shots: [{from: 0, k: 1.0, kEnd: 1.08},
           {from: 96, only: 0, k: 1.28, kEnd: 1.4, tx: 520, ty: 1020}],
   flicks: [{at: 14, x: 560, y: 1140}],
   sfx: [{at: 12, name: "core_tiktok", vol: 0.3}, {at: 12, name: "sfx_whip", vol: 0.42}],
  ...
  {at: 5697,
   exhibit: "outro_plate",
   title: ["MARKET LESSONS", "WITH SOL", "ep.2 — sixteen minutes"], actors: []},
];

// Frame, relative to a beat's start, at which its exhibit becomes visible.
const exhibitFromOf = (b: Beat) =>
  (b.shots ?? []).find((s) => !s.tvPose && !s.hideCard)?.from ?? 0;
...
// Hold only an image plate - the code-drawn graphics animate off their own
// beat clock and cannot be held past it.
...
const beatAt = (f: number) => {
  ...
  return {cur, idx: i, since: f - cur.at,
    hold: (next ? next.at : FAIRMARKET_FRAMES) - cur.at};
};
...
const Char: React.FC<{a: Actor; since: number; frame: number; speaking: boolean;
  ...
  // CAST at one point, so beats keep their authored relative sizes.
  const ah = a.h * CAST;
...
const TVPose: React.FC<{pose: string; since: number}> = ({pose, since}) => {
  ...
  //
  // Scaling by HEIGHT (the old `WALL.h * 1.85 / d.h`) sizes a
    ...
    <div style={{position: "absolute", inset: 0, overflow: "hidden",
      ...
      <Img src={staticFile(d.src)}
        style={{position: "absolute", width: w, height: h,
          left: WALL.w / 2 - w / 2,
          top: WALL.h / 2 - h * 0.29 - drift + (1 - rise) * 30,
          opacity: rise}} />
    </div>
  );
};
          ...
          {shot?.tvPose ? <TVPose pose={shot.tvPose} since={shotSince} />
            : wallEx ? (
              <ExhibitPlate name={wallEx.name} since={frame - wallEx.start}
                w={WALL.w} h={WALL.h}
                callout={cur.exhibit ? cur.callout : undefined}
                dir={(wallEx.start / 90) % 2 < 1 ? 1 : -1} />
            ) : cur.graphic === "timeline_nvidia" ? (
              <TimelineExhibit since={since} w={WALL.w} h={WALL.h}
              ...
              <PerformanceExhibit since={since} w={WALL.w} h={WALL.h}
              ...
              <CounterExhibit since={since} w={WALL.w} h={WALL.h}
